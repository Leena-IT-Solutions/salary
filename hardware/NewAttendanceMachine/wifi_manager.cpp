...
static void startMDNSAndPortal(Config &cfg) {
    ...
    LOG_PRINTF("[mDNS] Terminal accessible at http://%s.local\n", domain);
}
...
void wifiStartAPMode(Config &cfg) {
    ...
    LOG_PRINTF("[WIFI AP] Access Point started: SSID='%s' | IP=192.168.4.1\n", apSsid.c_str());
...
void wifiConnect(Config &cfg) {
    if (strlen(cfg.wifi_ssid) == 0) {
        LOG_PRINTLN("[WIFI] No Wi-Fi SSID configured. Starting AP Mode.");
        wifiStartAPMode(cfg);
        return;
    }

    LOG_PRINTF("[WIFI] Connecting to Wi-Fi SSID: '%s'...\n", cfg.wifi_ssid);
    ...
    if (WiFi.status() == WL_CONNECTED) {
        apMode = false;
        disconnectedStart = 0;
        LOG_PRINTF("[WIFI] Connected cleanly! IP Address: %s | RSSI: %d dBm\n", 
                   WiFi.localIP().toString().c_str(), WiFi.RSSI());
        configTime(cfg.tz_offset, 0, "pool.ntp.org", "time.nist.gov");
        startMDNSAndPortal(cfg);
        beepSuccess();
    } else {
        LOG_PRINTF("[WIFI] Initial connection attempt to '%s' pending. Starting dual AP+STA mode...\n", cfg.wifi_ssid);
...
void wifiLoop(Config &cfg) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (apMode) {
            ...
            LOG_PRINTF("[WIFI] Re-connected to '%s'! IP: %s (SoftAP disabled)\n", 
    ...
    if (!apMode) {
        if (disconnectedStart == 0) {
            ...
            LOG_PRINTLN("[WIFI] Wi-Fi connection lost for >30s. Enabling fallback AP mode (192.168.4.1)...");
            wifiStartAPMode(cfg);
        }
    }
}
...
void wifiApplyLiveWifiCredentials(Config &cfg) {
    if (strlen(cfg.wifi_ssid) > 0) {
        LOG_PRINTF("[WIFI] Live applying new Wi-Fi credentials for SSID: '%s'\n", cfg.wifi_ssid);
        wifiConnect(cfg);
    }
}
