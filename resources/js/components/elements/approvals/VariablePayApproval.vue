<template>
    <div class="variable-pay-dashboard">
        <!-- Top bar search -->
            ...
            <div class="col-12 col-xl-4">
                <div v-if="employee" class="profile-card-modern shadow-sm sticky-top" style="top: 20px;">
                    ...
                    <div class="p-4 pt-0 text-center">
                         <div class="p-3 bg-soft-info rounded-4 text-info small">
                            <i class="bi bi-gift-fill me-2"></i>
                            Variable pay can include bonuses, incentives, or performance-based earnings.
                        </div>
                    </div>
                </div>

                <div v-else class="empty-state-card text-center p-5 shadow-sm bg-white rounded-4 border-dashed">
                    <i class="bi bi-currency-dollar text-muted display-4"></i>
                    <p class="mt-3 text-muted">Enter employee code to approve variable earnings.</p>
                </div>
            </div>
                ...
                <div v-if="(item && employee) || item.id != null" class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold mb-0">
                            {{ item.id ? 'Modify Variable Pay' : 'New Variable Pay Entry' }}
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <forms-select-field name="earning_id" label="Earning Type" v-model="item.earning_id" :error="errors.earning_id" classes="col-12 col-lg-6" 
                            :options="types"></forms-select-field>

                            <forms-date-field name="app_date" label="Effective Date" v-model="item.app_date" :error="errors.on_date" classes="col-12 col-lg-3"></forms-date-field>
                            <forms-text-field name="amount" label="Amount (₹)" v-model="item.amount" :error="errors.amt" classes="col-12 col-lg-3"></forms-text-field>
                            
                            <forms-select-field name="status" label="Status" v-model="item.status" :error="errors.status" classes="col-12 col-lg-4" 
                            :options="[{ key: 'Approved', val: 'Approved' }, { key: 'Rejected', val: 'Rejected' }, { key: 'Pending', val: 'Pending' }]"></forms-select-field>

                            <forms-text-field name="note" label="Incentive Description / Note" placeholder="e.g. Sales achievement bonus, Festival incentive..." v-model="item.note" :error="errors.note" classes="col-12 col-lg-8"></forms-text-field>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                            ...
                            <forms-submit-button name="" v-model="loading" label="Save Variable Pay" @click="save()" classes="btn-lg px-5"></forms-submit-button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-header bg-white border-0 py-3">
                        <div class="row align-items-center">
                            <div class="col text-start">
                                <h5 class="fw-bold mb-0">Variable Pay History</h5>
                            </div>
                            <div class="col-auto">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                <tr v-for="row in items" :key="row.id" class="cursor-pointer" @click="edit(row)">
                                    ...
                                    <td class="text-center small">{{ row.earning.name }}</td>
                                    <td class="text-center">
                                        <span class="fw-bold text-success">₹{{ row.amount }}/-</span>
                                    </td>
                                    <td class="text-center">
...
export default {
    ...
    methods: {
        ...
        edit(row){
            ...
            this.item.earning_id = row.earning_id;
            this.item.app_date = row.app_date;
            this.item.amount = row.amount;
            this.item.note = row.note;
        ...
        add(){
            ...
            const payload = {
                ...this.item,
                on_date: this.item.app_date,
                amt: this.item.amount
            };
            axios.post('/approvals/variable_pay/add', payload).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
        ...
        update(){
            ...
            axios.post('/approvals/variable_pay/update', payload).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
                if(err.response && err.response.status == 422){
