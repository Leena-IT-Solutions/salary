<template>
    <div class="shortleave-dashboard">
        <!-- Top bar search -->
        ...
        <div class="row g-4">
            ...
            <div class="col-12 col-xl-4">
                ...
                <div v-else class="empty-state-card text-center p-5 shadow-sm bg-white rounded-4 border-dashed">
                    ...
                    <p class="mt-3 text-muted">Enter employee code to manage short leaves.</p>
                </div>
            </div>
                ...
                <div v-if="(item && employee) || item.id != null" class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold mb-0">
                            {{ item.id ? 'Modify Short Leave' : 'New Short Leave Entry' }}
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <forms-date-field name="on_date" label="Date" v-model="item.on_date" :error="errors.on_date" classes="col-12 col-lg-4"></forms-date-field>
                            <forms-time-field name="from_time" label="From Time" v-model="item.from_time" :error="errors.from_time" classes="col-12 col-lg-4"></forms-time-field>
                            <forms-time-field name="to_time" label="To Time" v-model="item.to_time" :error="errors.to_time" classes="col-12 col-lg-4"></forms-time-field>
                            
                            <forms-select-field name="status" label="Status" v-model="item.status" :error="errors.status" classes="col-12 col-lg-6" 
                            :options="[{ key: 'Approved', val: 'Approved' }, { key: 'Rejected', val: 'Rejected' }, { key: 'Pending', val: 'Pending' }]"></forms-select-field>

                            <forms-select-field name="is_lop" label="Loss of Pay?" v-model="item.is_lop" :error="errors.is_lop" classes="col-12 col-lg-6" 
                            :options="[{ key: 'Yes', val: 'Yes' }, { key: 'No', val: 'No' }]"></forms-select-field>

                            <forms-text-field name="reason" label="Reason" v-model="item.reason" :error="errors.reason" classes="col-12"></forms-text-field>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                            ...
                            <forms-submit-button name="" v-model="loading" label="Save Short Leave" @click="save()" classes="btn-lg px-5"></forms-submit-button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-header bg-white border-0 py-3">
                        <div class="row align-items-center">
                            <div class="col text-start">
                                <h5 class="fw-bold mb-0">Short Leave History</h5>
                            </div>
                            <div class="col-auto">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light">
                                <tr class="text-uppercase small fw-bold text-muted">
                                    ...
                                    <th class="border-0 text-center">Timing</th>
                            ...
                            <tbody>
                                <tr v-for="row in items" :key="row.id" class="cursor-pointer" @click="edit(row)">
                                    ...
                                    <td class="text-center small">{{ row.from_time }} - {{ row.to_time }}</td>
...
export default {
    ...
    methods: {
        ...
        reset(){
            ...
            this.item.from_time = null;
            this.item.to_time = null;
        ...
        edit(row){
            ...
            this.item.from_time = row.from_time;
            this.item.to_time = row.to_time;
            this.item.reason = row.reason;
            this.item.status = row.status;
            this.item.is_lop = row.is_lop;
