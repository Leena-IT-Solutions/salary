            ...
            <div class="col-12 col-xl-4">
                <div v-if="employee" class="profile-card-modern shadow-sm sticky-top" style="top: 20px;">
                    ...
                    <div class="p-4 pt-0 text-center">
                         <div class="p-3 bg-soft-danger rounded-4 text-danger small">
                            <i class="bi bi-info-circle-fill me-2"></i>
                            Fines are permanent deductions and will reflect in the next payroll cycle.
                        </div>
                    </div>
                </div>
                ...
                <div v-else class="empty-state-card text-center p-5 shadow-sm bg-white rounded-4 border-dashed">
                    ...
                    <p class="mt-3 text-muted">Enter employee code to issue or manage fines.</p>
                </div>
            </div>
                ...
                <div v-if="(item && employee) || item.id != null" class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold mb-0">
                            {{ item.id ? 'Modify Fine Record' : 'Issue New Fine' }}
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <forms-date-field name="app_date" label="Fine Date" v-model="item.app_date" :error="errors.on_date" classes="col-12 col-lg-4"></forms-date-field>
                            <forms-text-field name="amount" label="Fine Amount (₹)" v-model="item.amount" :error="errors.fine_amt" classes="col-12 col-lg-4"></forms-text-field>
                            ...
                            <forms-text-field name="note" label="Reason for Fine" placeholder="e.g. Damage to company property, Disciplinary action..." v-model="item.note" :error="errors.reason" classes="col-12"></forms-text-field>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                            ...
                            <forms-submit-button name="" v-model="loading" label="Save Fine Record" @click="save()" classes="btn-lg px-5"></forms-submit-button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-header bg-white border-0 py-3">
                        <div class="row align-items-center">
                            <div class="col text-start">
                                <h5 class="fw-bold mb-0">Fine Issuance History</h5>
                            </div>
                            <div class="col-auto">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                <tr v-for="row in items" :key="row.id" class="cursor-pointer" @click="edit(row)">
                                    ...
                                    <td class="text-center fw-medium">{{ formatDate(row.app_date) }}</td>
                                    <td class="text-center">
                                        <span class="fw-bold text-danger">₹{{ row.amount }}/-</span>
                                    </td>
                                    <td class="text-center">
    ...
    methods: {
        ...
        add(){
            ...
            const payload = {
                ...
                fine_amt: this.item.amount,
                reason: this.item.note
            };
            axios.post('/approvals/fine/add', payload).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
        ...
        update(){
            ...
            axios.post('/approvals/fine/update', payload).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
                if(err.response && err.response.status == 422){
        ...
        deleteNow(){
            this.loading = true;
            axios.post('/approvals/fine/delete', this.item).then(res => {
                this.reset();
                this.search();
            }).finally(() => {
                this.loading = false;
        ...
        getEmployee(){
            ...
            if(this.employee_code){
                axios.get('/approvals/fine/employee/' + this.employee_code).then(res => {
