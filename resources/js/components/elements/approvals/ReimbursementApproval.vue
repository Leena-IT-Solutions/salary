            ...
            <div class="col-12 col-xl-4">
                <div v-if="employee" class="profile-card-modern shadow-sm sticky-top" style="top: 20px;">
                    ...
                    <div class="p-4 pt-0 text-center mt-3">
                         <div class="p-3 bg-soft-success rounded-4 text-success small">
                            <i class="bi bi-receipt me-2"></i>
                            Approved reimbursements will be processed in the upcoming salary disbursement.
                        </div>
                    </div>
                </div>
                ...
                <div v-else class="empty-state-card text-center p-5 shadow-sm bg-white rounded-4 border-dashed">
                    ...
                    <p class="mt-3 text-muted">Enter employee code to approve expense claims.</p>
                </div>
            </div>
                ...
                <div v-if="(item && employee) || item.id != null" class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold mb-0">
                            {{ item.id ? 'Modify Reimbursement Claim' : 'New Reimbursement Entry' }}
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <forms-select-field name="reimbursement_component_id" label="Expense Head" v-model="item.reimbursement_component_id" :error="errors.reimbursement_component_id" classes="col-12 col-lg-6" 
                            :options="types"></forms-select-field>

                            <forms-date-field name="app_date" label="Claim Date" v-model="item.app_date" :error="errors.app_date" classes="col-12 col-lg-3"></forms-date-field>
                            ...
                            <forms-text-field name="note" label="Expense Details / Remarks" placeholder="e.g. Travel to client site, Office stationary..." v-model="item.note" :error="errors.note" classes="col-12 col-lg-8"></forms-text-field>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                            ...
                            <forms-submit-button name="" v-model="loading" label="Approve Claim" @click="save()" classes="btn-lg px-5"></forms-submit-button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-header bg-white border-0 py-3">
                        <div class="row align-items-center">
                            <div class="col text-start">
                                <h5 class="fw-bold mb-0">Reimbursement Claims History</h5>
                            </div>
                            <div class="col-auto">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light">
                                <tr class="text-uppercase small fw-bold text-muted">
                                    ...
                                    <th class="border-0">Expense Head</th>
                            ...
                            <tbody>
                                <tr v-for="row in items" :key="row.id" class="cursor-pointer" @click="edit(row)">
                                    ...
                                    <td>
                                        <span class="small fw-medium">{{ row.reimbursement_component.name }}</span>
                                    </td>
...
export default {
    ...
    data(){
        return {
            ...
            item: {
                id: null,
                employee_id: null,
                reimbursement_component_id: null,
                app_date: null,
                amount: null,
                status: null,
                note: null,
    ...
    methods: {
        ...
        reset(){
            ...
            this.item.reimbursement_component_id = null;
        ...
        edit(row){
            ...
            this.item.reimbursement_component_id = row.reimbursement_component_id;
        ...
        fetch(){
            let url = '/approvals/reimbursement/fetch';
        ...
        add(){
            this.loading = true;
            axios.post('/approvals/reimbursement/add', this.item).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
        ...
        update(){
            this.loading = true;
            axios.post('/approvals/reimbursement/update', this.item).then(res => {
                this.reset();
                this.search();
            }).catch(err => {
                if(err.response && err.response.status == 422){
        ...
        deleteNow(){
            this.loading = true;
            axios.post('/approvals/reimbursement/delete', this.item).then(res => {
                this.reset();
                this.search();
            }).finally(() => {
                this.loading = false;
        ...
        getEmployee(){
            ...
            if(this.employee_code){
                axios.get('/approvals/reimbursement/employee/' + this.employee_code).then(res => {
