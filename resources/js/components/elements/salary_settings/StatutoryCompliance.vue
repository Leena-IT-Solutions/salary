<template>
    <div>
        ...
        <div class="py-3 px-4 border">
            <span class="m-0 h4 fw-bold">Statutory Compliance</span>
            <span class="float-end">
                <button 
                @click="isForm = !isForm"
                :class="[isForm ? 'btn-danger' : 'btn-outline-primary']"
                class="btn btn-sm">
                    {{ isForm ? 'Close Form' : 'Add Statutory Compliance' }}
                </button>
            </span>
        </div>
            ...
            <div class="row gx-5 gy-4">
                ...
                <div class="col-12 col-xl-6">
                    <div class="row g-4">
                        ...
                        <forms-text-field v-model="item.scheme_name" name="scheme_name" label="Scheme Name" error="" classes=""></forms-text-field>

                        <forms-text-field v-model="item.abbreviation" name="abbreviation" label="Abbreviation" error="" classes=""></forms-text-field>

                        <forms-text-field v-model="item.registration_number" name="registration_number" label="Registration Number" error="" classes=""></forms-text-field>
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th @click="orderBy('scheme_name')" class="cursor-pointer">Scheme Name</th>
                        <th @click="orderBy('registration_number')" class="cursor-pointer">Registration Number</th>
                ...
                <tbody>
                    <tr v-for="row in items" :key="row.id">
                        <td>{{ row.id }}</td>
                        <td><a :href="'/salary_settings/statutory_compliance/'+ row.id +'/condition/'">{{ row.scheme_name }} - {{ row.abbreviation }}</a></td>
...
export default {
    ...
    methods: {
        ...
        reset(){
            ...
            this.item.is_part_of_salary = null;
