...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('employees', function (Blueprint $table) {
            ...
            $table->enum('blood_group', ['O +ve', 'O -ve', 'A +ve', 'A -ve', 'B +ve', 'B -ve', 'AB +ve', 'AB -ve', 'HH', 'Other']);
            $table->enum('religion', ['Hindu', 'Muslim', 'Christian', 'Sikh', 'Buddhist', 'Jain', 'Atheist', 'Other']);
            $table->string('cast', 100)->nullable();
            $table->string('subcast', 100)->nullable();
            $table->string('mothertongue', 100);
            $table->string('nationality', 100);
            $table->enum('marital_status', ['Married', 'Widowed', 'Separated', 'Divorced', 'Single', 'Other']);
            $table->enum('qualification', ['Primary School', 'Secondary School', 'High School', 'Undergraduate', 'Graduate', 'Diploma', 'Masters', 'Doctorate', 'Other'])->nullable();
